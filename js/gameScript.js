...
function bindGameEvents() {
    ...
    document.addEventListener('pointerdown', function (event) {
        ...
        if (target && target.closest && target.closest('.pages, .optionButton, button, a, select, input, .submit, .restart')) {
            return;
        }
...
function activateBlackTile(tile) {
    if (!tile || !tile.id || tile.id === 'tileTemplate' || tile.classList.contains('black')) {
        return false;
    }
