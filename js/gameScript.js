...
function loadPlayerProgress() {
    var savedProgress;
    try {
        savedProgress = JSON.parse(localStorage.getItem('dontTapWhiteTiles.playerProgress') || '{}');
    } catch (error) {
        savedProgress = {};
    }
    playerExp = Math.max(0, Number(savedProgress.exp) || 0);
    playerLevel = Math.max(1, Math.floor(Number(savedProgress.level) || 1));
    normalizePlayerProgress();
}

function normalizePlayerProgress() {
    var expRequired = getExpRequiredForLevel(playerLevel);
    while (playerExp >= expRequired) {
        playerExp -= expRequired;
        playerLevel += 1;
        expRequired = getExpRequiredForLevel(playerLevel);
    }
}

function savePlayerProgress() {
    localStorage.setItem('dontTapWhiteTiles.playerProgress', JSON.stringify({
        exp: playerExp,
        level: playerLevel
    }));
}
...
function addPlayerExp(amount) {
    playerExp += Math.max(0, Number(amount) || 0);
    normalizePlayerProgress();
    savePlayerProgress();
    updatePlayerProgressDisplay();
}
...
function updatePlayerProgressDisplay() {
    var expRequired = getExpRequiredForLevel(playerLevel);
    var percent = expRequired ? (playerExp / expRequired) * 100 : 0;
    $('#player-level').text(playerLevel);
    $('#player-exp-label').text(playerExp.toFixed(2) + ' / ' + expRequired + ' EXP');
    $('#player-exp-percent').text(percent.toFixed(2) + '%');
    $('#player-exp-fill').css('width', Math.min(100, percent) + '%');
}
