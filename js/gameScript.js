...
function bindGameEvents() {
    ...
    document.addEventListener('pointerdown', function (event) {
        ...
        event.preventDefault();
        handleWrongTap(event.clientX, event.clientY, null);
    }, true);
...
function populateBlackTiles() {
    var tiles = $('#gameContainer .square').not('.black');
    var count = Math.min(blackTileLimit, tiles.length);
    while (count > 0) {
        var pick = Math.floor(Math.random() * tiles.length);
        var tile = tiles[pick];
        if (tile && tile.id !== 'tileTemplate' && tile.id !== ('tile-' + lastRemovedBlackTileId)) {
            activateBlackTile(tile);
        }
        tiles = $('#gameContainer .square').not('.black');
        count--;
    }
}
...
function refreshBlackTilePopulation() {
    ...
    while (activeBlackTiles < blackTileLimit) {
        var candidates = $('#gameContainer .square').not('.black');
        var safeCandidates = [];
        candidates.each(function () {
            if (this.id !== 'tile-' + lastRemovedBlackTileId) {
...
function saveHistory(stats, win) {
    var currentEntries = JSON.parse(localStorage.getItem('dontTapWhiteTiles.history') || '[]');
    var playerName = $('#name').val() || 'Player';
    var entry = {
        id: playerName,
...
function handleEndGame(win) {
    if ($('.endgame:visible').length) {
        if ($('#name:visible').length && !$('#name').val().trim()) {
            return false;
        }
        $('.restart').css('display', 'flex');
        $('.endgame').hide();
