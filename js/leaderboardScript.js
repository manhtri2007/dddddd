...
function renderLeaderboard(entries) {
    ...
    var totalScore = list.reduce(function (total, item) {
        return total + Number(item.totalScore || 0);
    }, 0);
    var totalBlackTiles = list.reduce(function (total, item) {
        return total + Number(item.blackTilesClicked || 0);
    }, 0);
    var totalDuration = list.reduce(function (total, item) {
        return total + Number(item.timeAlive || 0);
    }, 0);
    $('#historySummary').text('Lượt chơi: ' + latestPlay + ' / Tổng số lượt: ' + totalPlays);
    $('#historyTotals').text('Tổng point: ' + totalScore.toFixed(2) + ' | Tổng tile đen: ' + totalBlackTiles + ' | Tổng thời lượng: ' + totalDuration + 's');
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        temp.find('.leaderboardTime').text(Number(item.totalScore || 0).toFixed(2) + ' pt | Đen: ' + Number(item.blackTilesClicked || 0));
...
function showHistoryDetails(id) {
    ...
    var details = [
        ...
        ['Tile đen đã bấm', Number(entry.blackTilesClicked || 0)],
